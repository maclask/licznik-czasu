*Licznik czasu ("Time Counter") is a browser-based stopwatch built for timing speeches in Oxford-style debate tournaments, tailored to the rules of the Polish Oxford Debate Championships (Mistrzostwa Polski Debat Oksfordzkich). It runs a main speech timer alongside an independent "ad vocem" rebuttal timer and a "Joker" timer, with audio cues, a fullscreen presentation mode, and configurable sponsor/organizer logos and motion text — all as a single static HTML page with no backend or account required.*

# Licznik czasu

Licznik czasu to działający w przeglądarce stoper stworzony do odmierzania czasu mów podczas debat w stylu oksfordzkim, dostosowany do standardów Mistrzostw Polski Debat Oksfordzkich. Obsługuje licznik główny wraz z niezależnym licznikiem ad vocem oraz licznikiem Jokera, oferując sygnalizację dźwiękową, tryb pełnoekranowy oraz konfigurowalne loga sponsorów/organizatorów i tezę debaty — a wszystko to jako pojedyncza statyczna strona HTML, bez backendu i bez potrzeby zakładania konta.
